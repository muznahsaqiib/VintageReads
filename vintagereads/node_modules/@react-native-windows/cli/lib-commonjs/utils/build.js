...
async function buildSolution(buildTools, slnFile, buildType, buildArch, msBuildProps, verbose, target, buildLogDirectory, singleproc) {
    const minVersion = new version_1.default(10, 0, 22621, 0);
    const allVersions = msbuildtools_1.default.getAllAvailableUAPVersions();
    if (!allVersions.some(v => v.gte(minVersion))) {
        throw new telemetry_1.CodedError('MinSDKVersionNotMet', 'Must have a minimum Windows SDK version 10.0.22621.0 installed');
    }
    ...
    if (typeof options.msbuildprops === 'string') {
        ...
        for (const prop of props) {
            ...
            if (propAssignment.length === 2 && propAssignment[0].trim().length > 0) {
                ...
                (0, commandWithProgress_1.newWarn)(`Unable to parse msbuildprop: '${prop}'`);
            }
        }
    }
    return result;
